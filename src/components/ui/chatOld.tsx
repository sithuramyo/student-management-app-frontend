...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const sortedNewMessages = [...initialMessages]
    ...
    setMessages(prev => {
      const isDuplicate = sortedNewMessages.every(newMsg =>
  ...
  useEffect(() => {
    if (messagePage === 1 && messageBoxRef.current) {
      const container = messageBoxRef.current;
      ...
      {!selectedUser ? (
        <div className="p-4 space-y-4 h-full flex flex-col">
          ...
          <ScrollArea className="h-[300px] w-[285px]">
            <div className="space-y-2">
              {isUsersLoading ? (
                <div className="text-sm text-gray-400">...<LoadingButton/>...</div>
              ) : (
        ...
        <div className="flex flex-col h-full">
          ...
          <div
            ...
            className="custom-scrollbar p-4 overflow-y-auto flex-1 flex flex-col gap-2"
          >

            {isMessagesLoading && messagePage === 1 ? (
              <div className="text-sm text-gray-400 text-center">...<LoadingButton/>...</div>
            ) : (
              <>
                {isLoadingMore && (
